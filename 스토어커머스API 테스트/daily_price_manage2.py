...
soldout_list = []                  #품절상품 목록 저장 리스트
new_soldoutlist = []               #신규품절 목록 저장 리스트
soldout_release_list = []          #품절해제 목록 저장 리스트
...
for j in range(start+2,paging+2):
    if waa.cell(j,9).value is not None:     # 현재의 가격이 None 이 아닐경우에만 실행
        ...
        if cp_soldout == 'x' and ss_soldout == 'empty':
            # print(f"{j-1} 품절상태 동일")
            #품절목록 리스트에 추가
            soldout_list.append(wbb.cell(j,9).value)
            logging.debug(f"{j-1} 품절상태. 상품이름: {wbb.cell(j,9).value} link: {wbb.cell(j,5).value}")
            sold_out_count += 1
        else:
            if cp_soldout == 'x':
                print(f"{j-1} 신규품절. 상품이름: {wbb.cell(j,9).value} link: {wbb.cell(j,5).value}")
                #신규품절목록 리스트에 추가
                new_soldoutlist.append(wbb.cell(j,9).value)
                ...
                #품절풀림목록 리스트에 추가
                soldout_release_list.append(wbb.cell(j,9).value)
...
#저장된 품절목록 리스트 출력
print(f"품절목록 리스트 : {soldout_list}")
logging.debug(f"품절목록 리스트 : {soldout_list}")
#저장된 신규품절목록 리스트 출력
print(f"신규품절목록 리스트 : {new_soldoutlist}")
logging.debug(f"신규품절목록 리스트 : {new_soldoutlist}")
#저장된 품절풀림목록 리스트 출력
print(f"품절풀림목록 리스트 : {soldout_release_list}")
logging.debug(f"품절풀림목록 리스트 : {soldout_release_list}")
...
df_slist = pd.read_excel(slist, keep_default_na=False).fillna('')
for i, prod_num in enumerate(df_slist.iloc[4:, 0].astype(int)):
...
wf, we = load_workbook(front).active, load_workbook(backend).active
wf.delete_rows(3, 3)
wf.delete_rows(500, 500)
we.delete_rows(3, 500)

wf.parent.save(front)
we.parent.save(backend)
print(f" wf max row {wf.max_row} , we max_row {we.max_row}")