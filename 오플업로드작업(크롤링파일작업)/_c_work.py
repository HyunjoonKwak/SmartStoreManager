...
def make_image_name(list):
    img_src = 'https://cosmo2023.speedgabia.com/ople_omega3/'
...
def detail_page(list):
    ...
    img_src = '<img src="https://cosmo2023.speedgabia.com/ople_omega3/'
    ...
    for i, rd in enumerate(list['Prd_ID']):
        ...
        if self_crawl == False:
            check = file_check('./data_crawl/renamed/'+str(rd)+'-type.jpg')
        else :
            check = file_check('./data_collect/renamed/'+str(rd)+'-type.jpg')
        d[i] = '<center>'
        d[i] += event_img1                  # event img 1
        d[i] += empty
        d[i] += '<h2 style="text-align: center;"><b>' + df_first['Brand'][i]+ df_first['브랜드'][i] + '</b></h2>'
        d[i] += empty
        d[i] += empty
        d[i] += empty
...
def make_final_upload_format(list, final):
    if list['표시가격'][0] != '' :  #할인금액과 최종표시가격이 업데이트 된 상태에서만 작업할것.
        ...
        df_final['수입사'] = '미국'
        df_final['배송비 템플릿코드'] = 2554792
        df_final['A/S 템플릿코드'] = 2504051
        df_final['PC즉시할인 값'] = 17500
        df_final['PC즉시할인 단위'] = '원'
        df_final['모바일즉시할인 값'] = 17500
        ...
        # a = df_final['재고수량'].copy()
        b = df_final['PC즉시할인 값'].copy()
        c = df_final['모바일즉시할인 값'].copy()
        for i, so in enumerate(list['Soldout']):
            if(so == 'o'):
                # a[i] = 0
                b[i] = 1000
                c[i] = 1000
...
if self_crawl == True:
    list_file = "./data_collect/prod_result_ready.xlsx"
    first_file = "./data_collect/result_first.xlsx"
else:
    list_file = "./data_crawl/prod_result_ready.xlsx"
    first_file = "./data_crawl/result_first.xlsx"
...
if res == True :
    ...
    if self_crawl == True:
        if(test):
            path = './data_collect/final_result_'+date+'.xlsx'
        else:
            path = './data_collect/final_result'+'.xlsx'
    else : 
        if(test):
            path = './data_crawl/final_result_'+date+'.xlsx'
        else:
            path = './data_crawl/final_result'+'.xlsx'
